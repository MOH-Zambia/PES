...
matches = pd.read_csv('Stage_1_All_Within_HH_Matches.csv')
...
matches_8 = pd.merge(left=CEN,
                     right=PES,
                     how="inner",
                     left_on =['last_name_cen',  'first_name_cen', 'year_birth_cen', 'sex_cen', 'relationship_hh_cen', 'EAid_cen'],
                     right_on=['first_name_pes', 'last_name_pes', 'year_birth_pes', 'sex_pes', 'relationship_hh_pes', 'EAid_pes'])
...
# Remove matches rejected (clerical_match = 0) in CROW
df = df[(df['clerical_match'] == 1) | (df['clerical_match'].isnull())].drop(columns=['clerical_match'])
...
# Save
df3.to_csv('Stage_2_All_Within_EA_Matches.csv', header = True)