...
prev_matches = pd.read_csv(OUTPUT_PATH + 'Stage_1_All_Within_HH_Matches.csv')
...
matches_1 = pd.merge(left=CEN,
                     right=PES,
                     how="inner",
                     left_on=['names_cen', 'year_cen', 'month_cen', 'EAid_cen'],
                     right_on=['names_pes', 'year_pes', 'month_pes', 'EAid_pes'])

# Matchkey 2: Edit Distance < 2 + Year + Month + EA          
matches_2 = pd.merge(left=CEN,
                     right=PES,
                     how="inner",
                     left_on=['year_cen', 'month_cen', 'EAid_cen'],
                     right_on=['year_pes', 'month_pes', 'EAid_pes'])
...
matches_3 = pd.merge(left=CEN,
                     right=PES,
                     how="inner",
                     left_on=['names_cen', 'year_cen', 'sex_cen', 'EAid_cen'],
                     right_on=['names_pes', 'year_pes', 'sex_pes', 'EAid_pes'])
...
matches_5 = pd.merge(left=CEN,
                     right=PES,
                     how="inner",
                     left_on=['names_cen', 'sex_cen', 'relationship_cen', 'EAid_cen'],
                     right_on=['names_pes', 'sex_pes', 'relationship_pes', 'EAid_pes'])

# Matchkey 6: Allowing last name to be different
matches_6 = pd.merge(left=CEN,
                     right=PES,
                     how="inner",
                     left_on=['forename_cen', 'year_cen', 'sex_cen', 'relationship_cen', 'EAid_cen'],
                     right_on=['forename_pes', 'year_pes', 'sex_pes', 'relationship_pes', 'EAid_pes'])

# Matchkey 7: Allowing first name to be different
matches_7 = pd.merge(left=CEN,
                     right=PES,
                     how="inner",
                     left_on=['last_name_cen', 'year_cen', 'sex_cen', 'relationship_cen', 'EAid_cen'],
                     right_on=['last_name_pes', 'year_pes', 'sex_pes', 'relationship_pes', 'EAid_pes'])
...
matches_9 = pd.merge(left=CEN,
                     right=PES,
                     how="inner",
                     left_on=['year_cen', 'sex_cen', 'relationship_cen', 'EAid_cen'],
                     right_on=['year_pes', 'sex_pes', 'relationship_pes', 'EAid_pes'])
