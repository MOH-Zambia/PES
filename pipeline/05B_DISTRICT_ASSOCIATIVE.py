...
df = pd.read_csv(CHECKPOINT_PATH + 'Stage_5_Within_District_Checkpoint.csv')

# --------------------------- CLERICAL MATCHING -----------------------------------#

# Open clerical results from CROW
clerical_results = pd.read_csv(CLERICAL_PATH + 'Stage_5_Within_DS_Matchkey_Clerical_DONE.csv')
...
clerical_results.to_csv(CLERICAL_PATH + 'Stage_5_Within_DS_Matchkey_Clerical_Reformatted.csv', index=False)
...
prev_matches = pd.read_csv(OUTPUT_PATH + 'Stage_4_Clerical_Search_EA_Matches.csv')
df3 = pd.concat([prev_matches, df, df2])

# Save
df3.to_csv(OUTPUT_PATH + 'Stage_5_All_Within_DS_Matches.csv', header=True, index=False)
